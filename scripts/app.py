...
# Import Matching Service (in project root: services/matching)
try:
    import sys
    _app_dir = os.path.dirname(os.path.abspath(__file__))
    _project_root = os.path.dirname(_app_dir)  # Go up from Scripts to project root
    matching_path = os.path.join(_project_root, "services", "matching")
        ...
        if os.path.exists(output_dir):
            ...
            if os.path.exists(nsf_all):
                try:
                    ...
                    app.logger.info(f"Loaded matching service ({version_str}) from {nsf_all}")
            ...
            if os.path.exists(mvp_file):
                try:
                    ...
                    app.logger.info(f"Loaded matching service ({version_str}) from {mvp_file}")
                    return _matching_service
                except Exception as e:
            ...
            if latest_file:
                try:
                    ...
                    app.logger.info(f"Loaded matching service ({version_str}) from {latest_file}")
                except Exception as e:
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            # But since SMTP isn't configured for most deployments, we'll be honest
            ...
            if smtp_configured:
                ...
                flash("No account found with that email address. Please check your email or sign up.", "error")
        ...
        if smtp_configured:
            ...
            if email_sent:
                ...
                # Email failed to send - show link on page as fallback
                flash("Email could not be sent. Please use the link below to reset your password.", "warning")
        
        # SMTP not configured or email failed - always show reset link on page
