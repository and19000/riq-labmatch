#!/usr/bin/env python3
"""
Faculty Data Pipeline v4.4 - Checkpoint Resume Edition

NEW FEATURES:
1. Checkpoint resume - pick up where you left off
...
CHECKPOINT FILES:
- checkpoint_phase1_openalex.json - Faculty extraction
- checkpoint_phase2a_directories.json - Directory scraping  
- checkpoint_phase2b_websites.json - Website discovery
- checkpoint_phase3a_orcid.json - ORCID emails
- checkpoint_phase3b_emails.json - Website emails
- checkpoint_final.json - Complete results

Usage:
    # Fresh run
    python faculty_pipeline_v4_4.py --institution harvard --max-faculty 600
...
class FacultyMember:
    ...
    def from_dict(cls, data: Dict) -> 'FacultyMember':
        ...
        return cls(
            ...
            research=ResearchProfile.from_dict(research_data),
            email=EmailData.from_dict(email_data),
            website=WebsiteData.from_dict(website_data),
...
class CheckpointManager:
    """Manages saving and loading checkpoints for pipeline resume."""
    ...
    def get_latest_phase(self) -> Optional[str]:
        """Get the most recent completed phase."""
    ...
    def clear(self) -> None:
        """Clear all checkpoints for this institution."""
    ...
    def save_faculty_list(self, phase: str, faculty_list: List[FacultyMember], extra_data: Dict = None) -> None:
        """Save faculty list as checkpoint."""
        data = {
    ...
    def load_faculty_list(self, phase: str) -> Optional[List[FacultyMember]]:
        """Load faculty list from checkpoint."""
...
SOFT_DENYLIST = [
    ...
    "aminer.org",  # Added - was causing issues
]
...
GENERIC_EMAIL_PATTERNS = [
    ...
    r'^ogephd@', r'^dms@', r'^hms@',  # Added - program emails
]
...
class BraveSearchClient:
    """Brave Search API client with quota checking."""
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if response.status_code == 402:
                    ...
                    self.queries_failed += 1
                    logger.error("⚠️ Brave API quota exhausted! Add more credits.")
                    return []
...
class OrcidEmailExtractor:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            "Accept": "application/json",
            "User-Agent": "FacultyPipeline/4.4"
        })
...
class DirectoryScraper:
    ...
    def _normalize_name(self, name: str) -> str:
        name = re.sub(r'\b(Dr|Prof|Professor|Mr|Mrs|Ms|PhD|MD|Jr|Sr|III|II)\b\.?', '', name, flags=re.IGNORECASE)
                ...
                if href.startswith('mailto:'):
                    ...
                    if self._is_valid_email(email):
                        ...
                        if parent:
                            ...
                            norm_name = self._normalize_name(name_context)
    ...
    def lookup_email(self, faculty_name: str) -> Optional[EmailData]:
        variations = self._generate_name_variations(faculty_name)
        for variant in variations:
            if variant in self.email_cache:
                return EmailData(
                    ...
                    extraction_method="directory_scrape",
                    name_match_score=1.0 if variant == variations[0] else 0.8,
                )
        return None
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "website_cache": self.website_cache,
        }
...
class WebsiteFinder:
    ...
    def _classify_page_type(self, url: str) -> Tuple[str, float]:
        ...
        for domain in SOFT_DENYLIST:
            if domain in url_lower:
                return "aggregator", -0.4  # Increased penalty
    ...
    def _score_result(self, result: Dict, name: str) -> Tuple[float, List[str], str]:
        ...
        if inst_domain and inst_domain in url:
            score += 0.3  # Increased bonus for institution domain
    ...
    def find_website(self, faculty: FacultyMember) -> Optional[WebsiteData]:
        ...
        for query in queries:
            ...
            if self.brave.quota_exhausted:
                logger.warning("Quota exhausted during search - stopping")
                break
    ...
    def find_websites_batch(
        ...
        directory_scraper: Optional[DirectoryScraper] = None,
        ...
        if not has_quota:
            ...
            logger.error("Please add credits at https://brave.com/search/api/")
            return faculty_list
        
        logger.info("✓ Brave API quota available")
        
        # Apply directory cache first
        ...
        for i, faculty in enumerate(eligible):
            if self.brave.quota_exhausted:
                logger.warning(f"⚠️ Quota exhausted at {i}/{len(eligible)} faculty")
                break
            ...
            if (i + 1) % 25 == 0:
                logger.info(f"\n--- Progress: {i+1}/{len(eligible)}, Found: {found}, "
                           f"Queries: {self.brave.queries_used} ---\n")
...
class WebsiteEmailExtractor:
    ...
    def _calculate_name_match_score(self, email: str, faculty_name: str) -> float:
        ...
        name_parts = [p for p in name_parts if p not in ['dr', 'prof', 'md', 'phd'] and len(p) > 1]
    ...
    def _find_contact_pages(self, soup: BeautifulSoup, base_url: str) -> List[str]:
        ...
        for a in soup.find_all('a', href=True):
            ...
            is_contact = is_contact or any(w in text for w in ['contact', 'email', 'reach'])
    ...
    def _select_best_email(self, candidates: List[Tuple[str, str]], faculty_name: str) -> Optional[Tuple[str, str, float]]:
        ...
        for email, method in candidates:
            ...
            name_score = self._calculate_name_match_score(email, faculty_name)
            ...
            if name_score >= 0.3 or total_score >= 0.4:
    ...
    def extract_emails_batch(self, faculty_list: List[FacultyMember]) -> List[FacultyMember]:
        logger.info("=" * 60)
        logger.info("PHASE 3B: Website Email Extraction")
        logger.info("=" * 60)
...
class OpenAlexClient:
    ...
    def __init__(self, contact_email: str):
        ...
        self.session = requests.Session()
        self.session.headers.update({
            "User-Agent": f"FacultyPipeline/4.4 (mailto:{contact_email})"
        })
...
class FacultyPipeline:
    ...
    def run(
        ...
        logger.info("=" * 70)
        logger.info(f"FACULTY PIPELINE v4.4 - {institution_name}")
        logger.info("=" * 70)
        logger.info(f"Resume mode: {resume}")
        logger.info(f"Only websites: {only_websites}")
        logger.info(f"Only emails: {only_emails}")
        logger.info("=" * 70)
        ...
        # Determine what to run based on resume/only flags
        run_phase1 = True
        ...
        # Override for only_* flags
        if only_websites:
            run_phase1 = run_phase2a = run_phase3a = run_phase3b = False
            run_phase2b = True
        ...
        for f in faculty_list:
            if f.email.value:
                email_sources[f.email.source.value if isinstance(f.email.source, DataSource) else f.email.source] += 1
...
def main():
    parser = argparse.ArgumentParser(description="Faculty Pipeline v4.4 - Checkpoint Resume Edition")
    ...
    parser.add_argument("--only-websites", action="store_true", help="Only run website discovery (requires checkpoint)")
    parser.add_argument("--only-emails", action="store_true", help="Only run email extraction (requires checkpoint)")
    parser.add_argument("--clear-checkpoints", action="store_true", help="Clear existing checkpoints before running")
